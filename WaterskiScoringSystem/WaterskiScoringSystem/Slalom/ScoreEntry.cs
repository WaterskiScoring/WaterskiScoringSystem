...
namespace WaterskiScoringSystem.Slalom {
    public partial class ScoreEntry : Form {
        ...
        private void ResizeWide_Click(object sender, EventArgs e) {
            ...
            TourEventRegDataGridView.Width = 425;
        }
            ...
            try {
                ...
                if ( curUpdateStatus.ToUpper().Equals( "Y" ) ) {
                    try {
                        ...
                        rowsProc = 0;
                        Int64 curScorePK = 0;
                        String curStartLen = "", curStatus = "TBD", curBoat = "", curScorePassNum = "", curEventClass = "" ;
                        ...
                        if ( curTimeInTol.Equals( "Y" ) ) {
                            ...
                            if (rowsProc > 0) {
                                ...
                                String curCheckRecordMsg = myCheckEventRecord.checkRecordSlalom( curAgeGroup, scoreTextBox.Text );
        ...
        private void setSlalomScoreEntry( DataGridViewRow inTourEventRegRow, int inRound ) {
            ...
            DataTable curMaxSpeedDataTable = getMaxSpeedData( curAgeGroup );
            if ( curMaxSpeedDataTable.Rows.Count > 0 ) {
                ...
                if ( myScoreDataTable.Rows.Count > 0 ) {
                    ...
                    myScoreRow = null;
                    DataRow[] curLineRow = SlalomLineSelect.myDataTable.Select( "ListCodeNum = 18.25" );
                    //DataRow[] curSpeedRow = SlalomSpeedSelect.myDataTable.Select( "ListCodeNum = " + curMaxSpeed.ToString() );
                    ...
                    nopsScoreTextBox.Text = "";
                    noteTextBox.Text = "";
                    TourBoatTextbox.Text = (String)listApprovedBoatsDataGridView.Rows[myBoatListIdx].Cells["BoatCode"].Value;
                    scoreEntryBegin();
                    loadBoatTimeView( curMaxSpeed );
                }
            } else {
                MessageBox.Show( "Maximum slalom speed not found for age division " + curAgeGroup );
            }
        }
        ...
        private void addRecapRow() {
            ...
            if ( curAddPass ) {
                ...
                curSkierPass = Convert.ToInt16( myRecapRow.Index + 1 );
                myRecapRow.Cells["PassNumRecap"].Value = curPassNum.ToString( "#0" );
                myRecapRow.Cells["skierPassRecap"].Value = curSkierPass.ToString( "#0" );
        ...
        private Boolean nextPassWithOption( Int16 curPassSpeed, DataRow curClassRow ) {
            Boolean curReturnValue = true;
            Byte curPassNum = 0;
            Int16 curMaxSpeed = 0, curSpeed = 0;
            Decimal curPassLine = 23M;

            String curOptUpType = "next";
            curMaxSpeed = SlalomSpeedSelect.MaxValue;
            curPassNum = (byte) (Decimal) myPassRow["ListCodeNum"];
            curSpeed = Convert.ToInt16((Decimal) myPassRow["MaxValue"]);
            if ( curSpeed < curMaxSpeed ) {
                OptUpDialogForm.showRuleMsg();
                                        ...
                                        if (curJudgeScore > 6) {
                                            ...
                                            if (posDelim > -1) {
                                                if (curJudgeEntry.Substring( posDelim ).Equals( ".25" )
                                                    ...
                                                    myRecapRow.ErrorText = " Decimal value in " + curColName + " must only contain .0 or .25 or .5";
                                                    MessageBox.Show( myRecapRow.ErrorText );
                                                    e.Cancel = true;
                                                }
                                            }
                                        ...
                                        if (Convert.ToDecimal( myRecapRow.Cells["ScoreRecap"].Value ) == 6) {
                                            if (myRecapRow.Cells["BoatTimeRecap"].ReadOnly) {
                                                ...
                                                if ((String)myRecapRow.Cells["TimeInTolRecap"].Value == "Y") {
                                                    ...
                                                    if (myRecapRow.Cells["ScoreProtRecap"].Value == "Y") {
                                                        if (isExitGatesGood()) {
                                                            isAddRecapRowInProg = true;
                                                            Timer curTimerObj = new Timer();
                                                            curTimerObj.Interval = 5;
                ...
                if ( myNumJudges == 1 ) {
                    ...
                    if ( ( !( isObjectEmpty( myRecapRow.Cells["Judge1ScoreRecap"].Value ) ) )
                        ...
                        try {
                            ...
                            //if ( curJudgeScore[0] < 6 ) myRecapRow.Cells["GateExit1Recap"].Value = false;
                            //if ( curJudgeScore[1] < 6 ) myRecapRow.Cells["GateExit2Recap"].Value = false;
                            //if ( curJudgeScore[2] < 6 ) myRecapRow.Cells["GateExit3Recap"].Value = false;
                    ...
                    if ( ( !( isObjectEmpty( myRecapRow.Cells["Judge1ScoreRecap"].Value ) ) )
                        ...
                        try {
                            ...
                            //if ( curJudgeScore[0] < 6 ) myRecapRow.Cells["GateExit1Recap"].Value = false;
                            //if ( curJudgeScore[3] < 6 ) myRecapRow.Cells["GateExit2Recap"].Value = false;
                            //if ( curJudgeScore[4] < 6 ) myRecapRow.Cells["GateExit3Recap"].Value = false;
                        ...
                        if ( SlalomSpeedSelect.MaxValue == curSpeed ) {
                            if ( curSkierPassNum == 1 ) {
                                if (isDivisionIntl( curAgeGroup )) {
                                    ...
                                    curPassNumMinSpeed--;
                                } else if (isDivisionIntl( curAgeGroup )) {
                                }
                            } else {
                                if ( inScore == 6 ) {
                                    if (isDivisionIntl( curAgeGroup )) {
                                    ...
                                    if ( tmpPassRow != null ) {
                                        ...
                                        if ( SlalomSpeedSelect.MaxValue >= tmpSpeed ) {
                                            if (isDivisionIntl( curAgeGroup )) {
                                                //Reduce pass number for IWWF reduce by an additional 6 bouys for minimum rope length is 18.25M
                                                curPassNumMinSpeed--;
                                            }
        ...
        private void updateBoatSelect() {
            ...
            if ( myScoreRow != null ) {
                try {
                    ...
                    StringBuilder curSqlStmt = new StringBuilder( "" );
                    curSqlStmt.Append( "Update SlalomScore Set Boat = '" + TourBoatTextbox.Text + "'" );
                    curSqlStmt.Append( " Where PK = " + curScorePK.ToString() );
                    int rowsProc = DataAccess.ExecuteCommand( curSqlStmt.ToString() );
                    Log.WriteFile( curMethodName + ":Rows=" + rowsProc.ToString() + " " + curSqlStmt.ToString() );
                } catch ( Exception excp ) {
        ...
        private DataRow getPassRow( Int16 inSpeed, Decimal inPassLine ) {
            ...
            if ( curRowsFound.Length > 0 ) {
                curRow = curRowsFound[0];
                //curPassNum = Convert.ToInt16( (Decimal)myPassRow["ListCodeNum"] );
            } else {
                curRow = null;
            }
            return curRow;
        ...
        private void getSkierScoreByRound( String inMemberId, String inAgeGroup, int inRound ) {
            ...
            curSqlStmt.Append( ", Score, NopsScore, Rating, MaxSpeed, StartLen, StartSpeed, Boat, Status" );
            curSqlStmt.Append( ", FinalPassNum, FinalSpeedMph, FinalSpeedKph, FinalLen, FinalLenOff, FinalPassScore, Note, LastUpdateDate " );
        ...
        private void getPassData( String inListName ) {
            ...
            myPassDataTable = getData( curSqlStmt.ToString() );
        }

        private DataTable getMaxSpeedData( String inAgeGroup ) {
            StringBuilder curSqlStmt = new StringBuilder( "" );
        ...
        private Boolean isDivisionIntl(String inAgeGroup) {
            if (myDivisionIntlDataTable == null) {
                ...
                myDivisionIntlDataTable = getData( curSqlStmt.ToString() );
            }
            if (myDivisionIntlDataTable == null) {
                return false;
            } else {
