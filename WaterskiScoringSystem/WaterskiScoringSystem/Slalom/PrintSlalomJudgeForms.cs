...
namespace WaterskiScoringSystem.Slalom {
    class PrintSlalomJudgeForms {
        ...
        private Single DrawHeader() {
            ...
            if (myTourName.Length > 45) {
                int curDelim = myTourName.Substring( 0, 45 ).LastIndexOf( " " );
                if (curDelim > 0) {
                    curText = myTourName.Substring( 0, curDelim ) + "\n" + myTourName.Substring( curDelim );
                } else {
                    curText = myTourName.Substring( 0, 45 ) + "\n" + myTourName.Substring( 45 );
                }
            } else {
                curText = myTourName;
            }
            ...
            if (myShowHeader) {
                if (myReportHeader.Length > 1) {
                    ...
                    } else if (myReportHeader.Length > 58) {
                        int curDelim = myReportHeader.Substring( 0, 57 ).LastIndexOf( " " );
                        if (curDelim > 0) {
                            curText = myReportHeader.Substring( 0, curDelim ) + "\n" + myReportHeader.Substring( curDelim );
                        } else {
                            curText = myReportHeader.Substring( 0, 57 ) + "\n" + myReportHeader.Substring( 57 );
                        }
                    } else {
            ...
            if (myShowFooter) {
                if (myUsePageNum) {
                    ...
                    PrintText( DateString, myLeftMargin, curPosYFooter, 0, 0, PageStringFont, false, HorizontalAlignment.Left );
                    PrintText( PageString, myLeftMargin, curPosYFooter, myPageWidth, 0, 0, 0, PageStringFont, new SolidBrush( myReportFooterTextColor ), BuildFormat( HorizontalAlignment.Center ) );
