...
namespace WaterskiScoringSystem.Trick {
	class PrintTrickSkierSpecForm {
		...
		public PrintTrickSkierSpecForm() {
			myReportName = "PrintTrickSkierSpecForm";
			myReportHeader = "PrintTrickSkierSpecForm";
			myReportFooter = "";
			myTourRules = "";
			myTourName = "";
		...
		private void PrintReportEntry( DataRow curRow, int curPageSkierCount, Single curPosY ) {
			...
			Single[] curPagePos;
			Single curTextBoxHeight = Convert.ToSingle( 70 );
			...
			if ( curPageSkierCount == 0 ) {
				curPosX += Convert.ToSingle( 165 );
			} else {
				curPosX += curFormBoxWidth + Convert.ToSingle( 140 );
			}
		...
		private Single DrawHeader() {
			...
			Image curImageFile = global::WaterskiScoringSystem.Properties.Resources.TrickSkierSpecForm;
			...
			curPosX = myLeftMargin;
			curPosY = myTopMargin + Convert.ToSingle( 88.0 );
