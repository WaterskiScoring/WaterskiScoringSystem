...
namespace WaterskiScoringSystem.Common
{
	...
	#region "public section"
		...
		public FormPrinting(System.Windows.Forms.Control f)
		{
            ...
            AddDelegateToPrintControl( "System.Windows.Forms.TabPage", new ControlPrinting( PrintTabPage ) );
	...
	#region "Managing PrintDocument or TIF object"
		...
		private void pd_PrintPage(object sender, PrintPageEventArgs ev)
		{
			...
			// Print each control on the form
			Single globalExtendedHeight;
            string topControlType = _f.GetType().ToString();
            if ( topControlType.IndexOf( "TabControl" ) > 0 ) {
                TabControl curTabControl = (TabControl)_f;
                foreach ( TabPage curTabPage in curTabControl.TabPages ) {
                    //Single extraHeight = mp.BeginPrintUnit( y, 0 );   //Space required for tab page caption
                    //mp.DrawString( "xxxxx", printFont, Brushes.Black, x, y );
                    //mp.DrawLines( _Pen, x, y + curTabControl.Height, x + curTabControl.Width, y );
                    //mp.EndPrintUnit();
                    //extendedHeight += extraHeight;
                    
                    PrintControls( curTabPage, _MultiPage, _xform, y, out globalExtendedHeight );

                    if ( TabControlBoxed ) {
                        _MultiPage.DrawFrame( _Pen, _xform, y, curTabControl.Width, curTabControl.Height );
                    }

                    y += _MultiPage.UsablePageHeight;
                }
            } else {
                PrintControls( _f, _MultiPage, _xform, y, out globalExtendedHeight );
            }
			...
			ref Single extendedHeight, out bool ScanForChildControls)
		{
			...
			mp.DrawString(ReportTitle, printFont, Brushes.Black, x, y, c.Width, fontHeight);
			y += fontHeight + 3;
	...
	#region "Recursive printing of controls"
			...
			out Single extendedHeight)
		{
            ...
            if ( c.Visible == true || topControlType.IndexOf( "TabControl" ) > 0 ) {
                bool scanForChildControls;
                try {
                    // Print my header
	...
	#region "Controls printing"
        ...
        public void PrintTabControl( System.Windows.Forms.Control c,
            ...
            switch ( typePrint ) {
                ...
                case ParentControlPrinting.AfterChilds:
                    if ( TabControlBoxed )
                        mp.DrawFrame( _Pen, x, y, tc.Width, tc.Height + extendedHeight );
                    break;
            }
             */
        }

        public void PrintTabPage( System.Windows.Forms.Control c,
            ParentControlPrinting typePrint,
            MultiPageManagement mp,
            Single x, Single y,
            ref Single extendedHeight, out bool ScanForChildControls ) {

            System.Windows.Forms.TabPage tp = (TabPage)c;
            System.Windows.Forms.TabControl tc = (TabControl)tp.Parent;
            ...
            switch ( typePrint ) {
                case ParentControlPrinting.BeforeChilds:
                    Single curY = y;
                    int curTabIndex = tc.TabPages.IndexOf( tp );
                    if ( curTabIndex > 0 ) {
                        curY += tc.Height;
                        extendedHeight += tc.Height * curTabIndex;
                    }
                    Single extraHeight = mp.BeginPrintUnit( curY, tc.ItemSize.Height );   //Space required for tab page caption
                    Font printFont = new Font( tp.Font.Name, tp.Font.Size, FontStyle.Bold );
                    Single h = mp.FontHeight( printFont );
                    if ( h > tc.ItemSize.Height )
                        h = tc.ItemSize.Height;
                    mp.DrawString( tp.Text, printFont, Brushes.Black, x, curY + ( tc.ItemSize.Height - h ) / 2, tp.Width, h );
                    mp.DrawLines( _Pen, x, curY + tc.ItemSize.Height, x + tc.Width, curY + tc.ItemSize.Height );
                    mp.EndPrintUnit();
                    extendedHeight += extraHeight;
                    break;
                case ParentControlPrinting.AfterChilds:
...
//		}
	...
	#region "Multi page class"
		public class MultiPageManagement 
		{
            ...
            public Single UsablePageHeight {
                get {
                    return _UsablePageHeight;
                }
                set {
                    _UsablePageHeight = value;
                }
            }
